...
syms u v w p q r x y z phi theta ps Ixx Iyy Izz fb1 fb2 fb3 mb1 mb2 mb3 mass t omega_ie

C_b_e = [1 0 0; 0 cos(phi) sin(phi);0 -sin(phi) cos(phi)]...
...
omega_ie_skew = formskewsym([0;0;omega_ie]);
I_mat = diag([Ixx Iyy Izz]);
omega_skew = formskewsym([p;q;r]);

vdot = C_b_e*([fb1;fb2;fb3]./mass) - 2*formskewsym(C_b_e*[p;q;r] - [0;0;omega_ie])*[u;v;w];
omega_dot = I_mat^-1*([mb1;mb2;mb3] - omega_skew*I_mat*[p;q;r]);
pdot = [u;v;w];
euler_rates = R_omega*(C_b_e*[p;q;r] - [0;0;omega_ie]);

F = [vdot;omega_dot;pdot;euler_rates];

J = jacobian(F,[u v w p q r x y z phi theta ps]);
J_num = subs(J,[u v w p q r x y z phi theta ps])
