classdef PropagateAero < handle
    ...
    methods
        function [obj,aeroForces,aeroMoments] = PropagateAero(density,state,latStick,longStick,pedalPosn,stripGeometry)

            [aeroForces,aeroMoments] = obj.stripTheory(density,state,latStick,longStick,pedalPosn,stripGeometry);

        end

        function [Strip_Theory_Forces,Strip_Theory_Moments] = stripTheory(obj,rho,X,latStick,longStick,pedalPosn,stripGeom)
            ...
            AOAi_LeftWing = obj.CalcInducedAOA(Vehicle.Aero.StripROM.LeftWing,AOA_p(LWIndices),V_p(LWIndices));
            AOAi_RightWing = obj.CalcInducedAOA(Vehicle.Aero.StripROM.RightWing,AOA_p(RWIndices),V_p(RWIndices));

            AOAi_LeftHT = obj.CalcInducedAOA(Vehicle.Aero.StripROM.LeftHT,AOA_p(LHTIndices),V_p(LHTIndices));
            AOAi_RightHT = obj.CalcInducedAOA(Vehicle.Aero.StripROM.RightHT,AOA_p(RHTIndices),V_p(RHTIndices));

            AOAi_VT = obj.CalcInducedAOA(Vehicle.Aero.StripROM.VT,AOA_p(VTIndices),V_p(VTIndices));
        ...
        function AOAi = CalcInducedAOA(obj,StripROM,AOA,StripVel)
