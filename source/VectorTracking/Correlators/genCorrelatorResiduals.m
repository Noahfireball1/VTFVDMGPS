function [resPsr,resCarr,varPsr,varCarr,newCN0,newAmplitude,newNoise] = genCorrelatorResiduals(refPsr,estPsr,refCarr,estCarr,oldCN0,oldAmplitude,oldNoise,activeSVIdx)
chipOffset = 0.5;
pdiTime = 1/50;
...
whiteNoise = randn(1);
midPhase = whiteNoise + carrFreqError/2;
phase = (whiteNoise + midPhase)/2;
...
noiseEstimate = 1./(2*pdiTime*oldCN0(activeSVIdx));

% Calculate Correlators
for i = 1:length(noiseEstimate)
    midIP(i) = calcCodeError(pdiTime/2,'I',codePhaseError(i),carrFreqError(i),0,midPhase(i),noiseEstimate(i),midAmplitude(i),sqrt(2));
    midQP(i) = calcCodeError(pdiTime/2,'Q',codePhaseError(i),carrFreqError(i),0,midPhase(i),noiseEstimate(i),midAmplitude(i),sqrt(2));
    IP(i) = calcCodeError(pdiTime/2,'I',codePhaseError(i),carrFreqError(i),0,whiteNoise,noiseEstimate(i),midAmplitude(i),sqrt(2));
    QP(i) = calcCodeError(pdiTime/2,'Q',codePhaseError(i),carrFreqError(i),0,whiteNoise,noiseEstimate(i),midAmplitude(i),sqrt(2));
...
newAmplitude = 0.99*oldAmplitude(activeSVIdx)' + 0.01*amplitude;
newNoise = 0.99*oldNoise(activeSVIdx)' + 0.01*gpsNoise;

newCN0 = (abs((newAmplitude - 4*newNoise)./(2*pdiTime*newNoise)));
