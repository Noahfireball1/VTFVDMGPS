...
for timeIdx = 2:length(time)
    ...
    %% Measurement Update
    [trueRange,trueRangeRate] = calcRange(truthStates(:,timeIdx),satStates);

    [psr,psrDot,unitVectors,activeSVs] = calcPsr(x_m(:,timeIdx),satStates);
    j = sum(activeSVs);

    [resPsr,resPsrDot,varPsr,varCarr,newCN0,newAmplitude,newNoise,newPhase] = ...
        genCorrelatorResiduals(trueRange(activeSVs),psr(activeSVs),...
        trueRangeRate(activeSVs),psrDot(activeSVs),...
